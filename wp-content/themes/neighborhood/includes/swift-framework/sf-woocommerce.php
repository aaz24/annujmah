<?php
	...
	*/
	...
	function neighborhood_custom_cart_button_text( $button_text ) {
		return $button_text ? $button_text : __( 'Add to Shopping Bag', 'swiftframework' );	 
	}
		...
		function sf_wishlist_button() {
			...
			if ( class_exists( 'YITH_WCWL_UI' ) )  {
				...
				$html  = '<div class="yith-wcwl-add-to-wishlist">';
				    ...
				    $html .= '><a href="' . htmlspecialchars($yith_wcwl->get_addtowishlist_url()) . '" data-product-id="' . $product->get_id() . '" data-product-type="' . $product_type . '" ' . $classes . ' ><i class="fa-star"></i></a>';
				    $html .= '</div>';
	...
	function sf_single_variation_add_to_cart_button() {
		...
		<div class="variations_button">
			<?php if ( !$catalog_mode ) { ?>
				...
				<button type="submit" data-product_id="<?php echo esc_attr($product->get_id()); ?>" data-quantity="1" data-default_text="<?php echo esc_attr($product->single_add_to_cart_text()); ?>" data-default_icon="sf-icon-add-to-cart" data-loading_text="<?php echo esc_attr($loading_text); ?>" data-added_text="<?php echo esc_attr($added_text); ?>" class="single_add_to_cart_button button alt"><span><?php echo esc_attr($product->single_add_to_cart_text()); ?></span></button>
